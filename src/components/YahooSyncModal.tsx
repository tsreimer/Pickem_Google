...
export const YahooSyncModal: React.FC = () => {
  ...
  const handleIngestRoster = () => {
    ...
    try {
      ...
      if (newTeams.length > 0) {
        ...
        setRosterImportStatus(`Successfully updated league with ${newTeams.length} actual teams from the Initech Invitational!`);
      }
    } catch {
  ...
  const handleParsePicks = async () => {
    ...
    setParseStatus('Parsing picks matrix...');
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setParseStatus(`Successfully parsed ${data.parsedCount} picks for the league!`);
      } else {
      ...
      <div className="bg-[#151D2A] border border-[#1E293B] rounded-2xl w-full max-w-4xl max-h-[90vh] p-6 shadow-2xl relative flex flex-col justify-between overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Target: <span className="text-purple-300 font-mono inline-flex items-center gap-1 font-semibold">The League (Initech Invitational)</span>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex gap-2 py-3 border-b border-[#1E293B] shrink-0 text-xs font-mono">
          ...
          >
            ...
            <span>2. Initech Invitational Picks</span>
          </button>
          <button
        ...
        {/* Modal Body */}
        <div className="flex-1 overflow-y-auto py-4 space-y-5 text-slate-300 text-xs leading-relaxed pr-2">
          
          {/* Canonical 5 Lock Windows Indicator Bar */}
          <div className="p-3 bg-[#0B0F17] rounded-xl border border-[#1E293B] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs">
              <span className="p-1 rounded bg-purple-950 border border-purple-800 text-purple-300">
                <Lock className="w-3.5 h-3.5" />
              </span>
              <span className="font-bold text-white">Yahoo Kickoff Lock Windows:</span>
            </div>
            <div className="flex flex-wrap items-center gap-1.5 font-mono text-[11px]">
              {yahooLockWindows.map((win) => {
                ...
                return (
                  ...
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${isSynced ? 'bg-emerald-400' : isLocked ? 'bg-amber-400' : 'bg-slate-500'}`} />
                    <span>{win.kickoffLabel}</span>
                    <span className="text-[10px] opacity-75">({isSynced ? 'Synced' : isLocked ? 'Locked' : 'Pending'})</span>
                  </span>
                );
              })}
            </div>
          ...
          {/* TAB 2: Initech Invitational Picks */}
          {activeTab === 'picks' && (
            <div className="space-y-4">
              
              {/* Synchronized Picks Table directly matching League Matrix */}
              <YahooGroupPicksTable />

              {/* League Access Status Card */}
              <div className="p-4 bg-[#0B0F17] rounded-xl border border-[#1E293B] space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="text-white font-bold text-sm flex items-center gap-2">
                        <span>League Direct Connection Status</span>
                      ...
                      <div className="text-slate-400 text-[11px]">
                        League: <strong className="text-white">Initech Invitational</strong> • Manager: <strong className="text-emerald-400">Todd Reimer</strong>
                      </div>
                    </div>
                  </div>
                  <a
                ...
                <div className="p-3 bg-slate-900/80 rounded-xl border border-slate-800 space-y-2 text-xs">
                  ...
                  <div className="pt-2 border-t border-slate-800 flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      placeholder="Enter Group Password (if set on the league)..."
              ...
              <div className="p-4 bg-[#0B0F17] rounded-xl border border-[#1E293B] space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-white font-bold text-sm">Active Roster: {teams.length} Teams in the League</div>
              ...
              {/* Ingest / Update Real Teams from the League */}
              <div className="p-4 bg-[#0B0F17] rounded-xl border border-[#1E293B] space-y-3">
...
8. Pigskin Oracle (Rachel)`}
              ...
              <div className="p-4 bg-[#0B0F17] rounded-xl border border-[#1E293B] space-y-3">
                ...
                <p className="text-slate-400 text-[11px]">
                  When picks lock in Yahoo (<code className="text-purple-300">football.fantasysports.yahoo.com/pickem</code>), paste lines like <span className="text-white font-mono">"Reimer Original: KC (14)"</span> to immediately update confidence points and scenario trees:
                </p>
                <textarea
                  value={pastedPicks}
...
Spread Hunter | DET (11)`}
          ...
          {activeTab === 'cron' && (
            <div className="space-y-4">
              {/* Game Lock Window Synchronization Control */}
              <div className="p-4 bg-[#0B0F17] rounded-xl border border-[#1E293B] space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-800">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-white font-bold text-sm">Post-Kickoff Lock Windows (Yahoo Sync)</span>
                        <span className="px-2 py-0.5 rounded bg-purple-500/20 text-purple-300 font-mono text-[10px] font-bold">
                          5 Canonical Slates
                        </span>
                      </div>
                      <p className="text-slate-400 text-[11px] mt-0.5">
                        Yahoo automatically locks games at kickoff. The sync daemon runs after each set locks to reveal opponent cards.
                      </p>
                    </div>
                  </div>

                  {/* Daemon Toggle & Global Sync Button */}
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={() => toggleAutoSync(!autoSyncEnabled)}
                      className={`px-3 py-1.5 rounded-lg border text-xs font-mono font-semibold flex items-center gap-1.5 transition ${
                        autoSyncEnabled
                    ...
                    >
                      ...
                      <span>Daemon: {autoSyncEnabled ? 'ACTIVE (60s)' : 'PAUSED'}</span>
                    </button>

                    <button
                      onClick={() => syncYahooLockWindow()}
                      disabled={isSyncingLockWindow}
                      className="px-3.5 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-black font-bold text-xs flex items-center gap-1.5 transition shadow-md shadow-emerald-500/20"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${isSyncingLockWindow ? 'animate-spin' : ''}`} />
                      <span>{isSyncingLockWindow ? 'Syncing...' : 'Sync All Windows'}</span>
                    </button>
                  </div>
                </div>

                {/* 5 Canonical Lock Windows Grid */}
                    ...
                    return (
                      <div
                        ...
                        className={`p-3.5 rounded-xl border transition-all flex flex-col justify-between ${
                          isSynced
                            ? 'bg-[#121824] border-emerald-500/30 shadow-sm'
                            : isLocked
                      ...
                      >
                        <div>
                          <div className="flex items-center justify-between gap-1 mb-1.5">
                            ...
                            >
                              {win.status === 'synced' ? 'Synced' : win.status === 'locked' ? 'Locked' : 'Pending'}
                            </span>
                          </div>

                          <div className="flex items-center gap-1.5 text-[11px] font-mono text-purple-300 mb-2">
                            <Clock className="w-3 h-3 text-purple-400" />
                            <span>{win.typicalKickoff}</span>
                          </div>

                          <div className="text-[11px] text-slate-400 mb-2 font-mono">
                            <span className="text-slate-300 font-bold">{win.gamesCount} Game{win.gamesCount > 1 ? 's' : ''}:</span>{' '}
                            {win.gamesList.join(', ')}
                          </div>

                          {win.lastSyncResult && (
                            <p className="text-[10px] text-slate-400 italic bg-black/40 p-2 rounded border border-slate-800/80 mb-2">
                              {win.lastSyncResult}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between text-[10px] font-mono text-slate-500">
                          <span>
                            {win.syncedAt
                              ? `Synced: ${new Date(win.syncedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
                              : 'Waiting for kickoff'}
                          </span>
                          <button
                            onClick={() => syncYahooLockWindow(win.id)}
                            disabled={isSyncingLockWindow}
                            className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-200 font-sans text-[11px] font-semibold transition"
                          >
                            Sync Slate
                          </button>
                        </div>
              ...
              {syncAuditLogs.length > 0 && (
                <div className="p-4 bg-[#0B0F17] rounded-xl border border-[#1E293B] space-y-2 font-mono text-xs">
                  <div className="flex items-center justify-between pb-1">
                    <span className="text-white font-bold flex items-center gap-1.5">
                      <Terminal className="w-3.5 h-3.5 text-emerald-400" />
                      <span>Yahoo Sync Audit Trail</span>
                    </span>
                    <span className="text-[11px] text-slate-500">Auto-logged on game lock events</span>
                  </div>
                  <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1">
                    {syncAuditLogs.map((log) => (
                      <div
                        key={log.id}
                        className="p-2 bg-slate-900/80 rounded border border-slate-800 flex items-center justify-between text-[11px]"
                      >
                        <div className="flex items-center gap-2">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                          <span className="text-slate-300 font-bold">{log.windowName}</span>
                          <span className="text-slate-400 text-[10px] hidden sm:inline">{log.message}</span>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="px-1.5 py-0.5 rounded bg-purple-900/50 text-purple-300 text-[10px]">
                            {log.triggerSource === 'auto_daemon' ? 'DAEMON' : 'MANUAL'}
                          </span>
              ...
              {/* Cadence Description Card */}
              <div className="p-4 bg-[#0B0F17] rounded-xl border border-[#1E293B] space-y-3">
                <div className="flex items-center gap-2">
                  <Cpu className="w-4 h-4 text-emerald-400" />
                  <span className="text-white font-bold text-sm">Automated Pipeline Cadence (Initech Invitational)</span>
                </div>
                ...
                <div className="space-y-2 pt-1 font-mono text-xs">
                  ...
                  <div className="p-2.5 bg-purple-950/40 rounded-lg border border-purple-800/60 flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-slate-400 ml-2">Locks Initech Invitational submitted cards</span>
                    </div>
        ...
        <div className="border-t border-[#1E293B] pt-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 text-xs text-slate-400 font-mono">
            ...
            <span>Initech Invitational Verified • SSL Encrypted</span>
          </div>
          <button
            onClick={() => setIsYahooSyncModalOpen(false)}
